    ...
    public HttpResponseMessage run(
        ...
        try {
            TopicManagementResponse response = FirebaseMessaging.getInstance()
                    .unsubscribeFromTopic(Arrays.asList(params.token), params.topic);
            logger.info(response.getSuccessCount() + " tokens were unsubscribed successfully.");
        } catch (FirebaseMessagingException e) {
            logger.info("Failed to send unsubscription request to Firebase Cloud Messaging. " + e.getMessage());
