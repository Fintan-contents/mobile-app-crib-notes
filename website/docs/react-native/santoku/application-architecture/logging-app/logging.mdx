...
```typescript フォーマット出力例
  // Error以外の場合
  //  [＜タイムスタンプ＞] [＜ログレベル＞] ＜ログのメッセージ内容＞
  log.info("ロード処理完了")
  //  -> "[2021-10-23T04:05:06.123+09:00] [info] ロード処理完了"

  // Errorの場合
  //   [＜タイムスタンプ＞] [＜ログレベル＞] [＜指定したエラーコード＞] ＜ログのメッセージ内容＞
  try {
    /* 中略 */
  } catch {
    log.error("xxx処理例外", "E001XXX")
    //  -> "[2021-10-23T04:05:06.123+09:00] [error] [E001XXX] xxx処理例外"
    /* 中略 */
  }
```
