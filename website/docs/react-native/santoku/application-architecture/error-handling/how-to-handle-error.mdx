...
ただし、非同期処理で発生したエラーに関しては、Firebase Crashlytics SDKで捕捉できないため、必ず個別にエラー処理を実施してください。
...
Reactコンポーネントでは、JSXのレンダリング処理や、コンポーネントのライフサイクルに応じた処理、`useEffect`内の処理などでエラーが発生します。

JSXのレンダリングでエラーが発生した場合は、[Error Boundary](https://ja.reactjs.org/docs/error-boundaries.html)でエラーを捕捉します。該当のコンポーネントを`Error Boundary`でラップしてエラー処理を実施してください。
...
JSXのレンダリング以外で発生したエラーをハンドリングする場合は、発生元が非同期処理（Promiseやawaitしている関数）かそれ以外の同期処理かで対応が変わります。

同期処理の場合は、`try...catch`文で囲んでエラーを捕捉します。
...
:::note
コンポーネントのライフサイクルに応じた処理や`useEffect`内の処理で発生したエラーも、`Error Boundary`で捕捉できます。しかし、Reactコンポーネントで発生したエラー全てを`Error Boundary`で捕捉してしまうと、個々のエラーに応じた処理を実施することが難しくなります。

そのため、JSXのレンダリング以外で発生したエラーについては`Error Boundary`を使用しないで、個別にエラー処理を実施します。
:::
