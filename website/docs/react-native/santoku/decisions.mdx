---
title: Decisions Records
sidebar_label: 目次
pagination_label: Decisions Records - 目次
hide_table_of_contents: true
---

<!-- textlint-disable ja-technical-writing/sentence-length,ja-technical-writing/max-comma,ja-spacing/ja-no-space-around-parentheses,jtf-style/3.3.かっこ類と隣接する文字の間のスペースの有無,ja-technical-writing/ja-no-mixed-period,ja-technical-writing/no-unmatched-pair,ja-technical-writing/no-doubled-joshi -->

import {PageList} from '../../../src/components';
const overviews = [
  {
    title: 'グローバルエラーハンドリング',
    to: '/react-native/santoku/decisions/adr-001-error-handling',
  },
  {
    title: 'HTTP通信で使用するライブラリ',
    to: '/react-native/santoku/decisions/adr-002-http-api-libraries',
  },
  {
    title: 'HTTP API通信で発生するエラーのハンドリング',
    to: '/react-native/santoku/decisions/adr-003-http-api-error-handling',
  },
  {
    title: 'ディープリンクの実現方式（廃止）',
    to: '/react-native/santoku/decisions/adr-004-deep-link',
  },
  {
    title: 'メッセージ管理の方針',
    to: 'react-native/santoku/decisions/adr-005-message',
  },
  {
    title: 'ログ出力の方針',
    to: 'react-native/santoku/decisions/adr-006-logging',
  },
  {
    title: '認証方式の方針',
    to: 'react-native/santoku/decisions/adr-007-auth',
  },
  {
    title: 'プッシュ通知方式の方針',
    to: 'react-native/santoku/decisions/adr-008-push-notification',
  },
  {
    title: 'FCMを用いたプッシュ通知の管理方針',
    to: 'react-native/santoku/decisions/adr-008-push-notification',
  },
  {
    title: 'プッシュ通知の内容に関する方針',
    to: 'react-native/santoku/decisions/adr-008-push-notification',
  },
  {
    title: 'キャッシュの取り扱いに関する方針',
    to: 'react-native/santoku/decisions/adr-011-cache-management',
  },
  {
    title: 'HTTP API通信に関する方針',
    to: 'react-native/santoku/decisions/adr-012-http-api',
  },
  {
    title: 'React Queryを用いた開発方針',
    to: 'react-native/santoku/decisions/adr-013-react-query',
  },
  {
    title: 'UIライブラリの選定',
    to: 'react-native/santoku/decisions/adr-014-ui-libraries',
  },
  {
    title: 'Androidのヘッドアップ通知',
    to: 'react-native/santoku/decisions/adr-015-android-head-up-notification',
  },
  {
    title: '強制アプリアップデート',
    to: 'react-native/santoku/decisions/adr-016-forced-app-updates',
  },
  {
    title: '地図表示ライブラリ',
    to: 'react-native/santoku/decisions/adr-017-map-view-library',
  },
  {
    title: 'ディープリンクの実現方式',
    to: '/react-native/santoku/decisions/adr-018-deep-link',
  },
  {
    title: 'ディープリンク受信時の画面遷移',
    to: 'react-native/santoku/decisions/adr-019-deep-link-navigation',
  },
]

このアプリのアーキテクチャや機能設計に関する判断の記録です。実際に方式を設計する際に、このサンプルアプリケーションをより有効に活用できるよう、設計した方式だけでなく検討過程も記録しています。

### 各判断にあたっての前提

各判断にあたり、このアプリの機能要件および非機能要件を満たすことを前提とします。

- [Requirements](./requirements.mdx)

<PageList overviews={overviews} colSize={12} />

<!-- textlint-enable ja-technical-writing/sentence-length,ja-technical-writing/max-comma,ja-spacing/ja-no-space-around-parentheses,jtf-style/3.3.かっこ類と隣接する文字の間のスペースの有無,ja-technical-writing/ja-no-mixed-period,ja-technical-writing/no-unmatched-pair,ja-technical-writing/no-doubled-joshi -->
