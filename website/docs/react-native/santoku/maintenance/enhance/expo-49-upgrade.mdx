...
:::info他のライブラリでサポートされていない`v5.2.2`が使われないようにするため、`^5.1.3`ではなく`~5.1.3`指定を推奨します。
...
- [expo-face-detector](https://docs.expo.dev/versions/latest/sdk/facedetector/)
  - Expo Goでの`expo-face-detector`のサポート削除されましたが、これまでと同様に、Expo Goの外部でライブラリを引き続き使用できます
...
#### (iOSシミュレータ)Expo GoのAppleシリコン上でのネイティブ実行サポート

TestFlightビルドを使用して、シミュレーターを使用せずにAppleシリコンMac上でExpo Goを直接実行もできます。
ただし、UIの問題がいくつかあり、修正中です。
...
新しいアーキテクチャをまだサポートしていない唯一のモジュールは`expo-updates`です。
...
`Constants.manifest`が非推奨です。代わりに、`Constants.expoConfig`を使用してください。
...
システムのデフォルト値を使うようになりました。

デバッグ ビルドでは明示的に`true`が設定されますが、他のビルドタイプ（例：リリースビルド）では指定されていません。
つまり、API 27以下ではデフォルトで`true`、API 28以降ではデフォルトで`false`になります。

セキュリティで保護されていないエンドポイントへのネットワークリクエストが存在する場合は、[expo-build-properties](https://docs.expo.dev/versions/v49.0.0/sdk/build-properties/)を使って`true`にする必要があります。
...
1. [既存のパッチファイルの更新](#既存のパッチファイルの更新)
1. [`jest-expo`のアップグレード](#jest-expoのアップグレード)
1. [`npx expo-doctor@latest`を実行してプロジェクトの依存関係に問題がないか確認](#npx-expo-doctorlatestを実行してプロジェクトの依存関係に問題がないか確認)
1. [tscチェック](#tscチェック)
   - [DimensionValueの厳密化](#dimensionvalueの厳密化)
   - [WebView.onScroll](#webviewonscroll)
   - [FunctionComponent](#functioncomponent)
1. [test修正](#test修正)
   - [`react-native-reanimated` `v3`対応](#react-native-reanimated-v3対応)
   - [`react-native`のDimensionsのmock失敗の対応](#react-nativeのdimensionsのmock失敗の対応)
   - [`jest.advanceTimersByTime`を`act`でwrap](#jestadvancetimersbytimeをactでwrap)
   - [`jest-mock`での`expo-constants`のdeprecated warn対応](#jest-mockでのexpo-constantsのdeprecated-warn対応)
   - [アニメーション中のAnimatedStyleの僅かな変化に対応](#アニメーション中のanimatedstyleの僅かな変化に対応)
   - [slideOutDurationテストの時間指定が厳密すぎるとテストが落ちる問題修正](#slideoutdurationテストの時間指定が厳密すぎるとテストが落ちる問題修正)
   - [keyExtractorの呼び出しが増えて落ちるようになったテスト修正](#keyextractorの呼び出しが増えて落ちるようになったテスト修正)
   - snapshot更新
1. [warn対応](#warn対応)
   - [TypeScriptバージョンに対応しているtypescript-eslintに更新](#typescriptバージョンに対応しているtypescript-eslintに更新)
   - [deprecated対応（JSX.Element → React.JSX.Element）](#deprecated対応jsxelement--reactjsxelement)
   - [initialScrollIndexの範囲外警告に対応](#initialscrollindexの範囲外警告に対応)
1. `npm run prebuild`を実行してネイティブプロジェクトの再生成
1. [buildエラー修正](#buildエラー修正)
   - [`app.notifee:core`を見つけられない問題対応](#appnotifeecoreを見つけられない問題対応)
1. renovate除外設定更新
   - 使用しなくなった`androidx.swiperefreshlayout:swiperefreshlayout`を削除
1. [不要になったConfig Plugins`withAndroidRemoveUsesClearTextTrafficForRelease`を削除](#不要になったconfig-pluginswithandroidremoveusescleartexttrafficforreleaseを削除)
1. [Proguardルールの更新](#proguardルールの更新)
   - [`react-native-svg`用ルールの削除](#react-native-svg用ルールの削除)
1. [Expoの更新履歴の確認と対応](#expoの更新履歴の確認と対応)
1. [React Nativeの更新履歴の確認と対応](#react-nativeの更新履歴の確認と対応)
1. [`expo-template-blank-typescript`の更新履歴の確認と対応](#expo-template-blank-typescriptの更新履歴の確認と対応)
1. [`expo-template-bare-minimum`の更新履歴の確認と対応](#expo-template-bare-minimumの更新履歴の確認と対応)
...
### `npx expo-doctor@latest`を実行してプロジェクトの依存関係に問題がないか確認

以下の実行結果のように`@expo/config-plugins`バージョンの問題が検知されました。
...
しかし、前回（Expo48への更新時）と同様に`@react-native-firebase`の更新が必要であるため保留としました。
...
そこで、`renderItem`に渡す関数には`React.FC`を使わないように変更しました。
...
#### `react-native`のDimensionsのmock失敗の対応
...
#### `jest.advanceTimersByTime`を`act`でwrap

以下の警告が出るようになっていたため、`act`でwrapするように変更しました。
...
#### `jest-mock`での`expo-constants`のdeprecated warn対応

[Expo SDK 49の主な変更 - Expoがサポートするライブラリや機能 - Expo Constants](#expo-constants)に記載した通り、Constants.manifestが非推奨になりました。
mockする際に[スプレッド構文](https://developer.mozilla.org/ja/docs/Web/JavaScript/Reference/Operators/Spread_syntax)を使ってアクセスすると`warn`ログが出力されるようになっています。
そのため、スプレッド構文ではなく[Proxy](https://developer.mozilla.org/ja/docs/Web/JavaScript/Reference/Global_Objects/Proxy)を使って上書きするようにしています。
...
2回目にindex=1が呼ばれるのは内部実装に依存していたため、`toHaveBeenNthCalledWith`の替わりに`toHaveBeenCalledWith`を使うようにしました。
...
#### initialScrollIndexの範囲外警告に対応

`react-native` `v0.72.0`から`FlatList`（`VirtualizedList`）の`initialScrollIndex`に`-1`（負の値）を指定するとwarnログがでるようになりました。
...
`expo-build-properties`プラグインの`android.extraMavenRepos[]`を追加する必要がありました。
...
### 不要になったConfig Plugins`withAndroidRemoveUsesClearTextTrafficForRelease`を削除
...
そのため、不要となったConfig Pluginsを削除しました。
...
そのため、`app.config.js`で`expo-build-properties`プラグインの`android.extraProguardRules`でルールを追加する必要はなくなりました。
...
### `expo-template-blank-typescript`の更新履歴の確認と対応
...
### `expo-template-bare-minimum`の更新履歴の確認と対応

[`expo-template-bare-minimum`の更新履歴](https://github.com/expo/expo/commits/sdk-49/templates/expo-template-bare-minimum)を確認しました。
...
- [Prebuild](../../development/build-configuration.mdx#prebuild)時に生成されないファイル
...
`MPL-2.0`は準コピーレフト型ライセンスであり使用しても問題ありません。
