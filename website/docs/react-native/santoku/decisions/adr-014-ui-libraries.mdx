...
UIライブラリの検討にあたり、いくつかの候補を挙げて比較してみます。
...
[^1]: テーマ機能とは、各コンポーネントや、ダークモード時のスタイル定義、デザイントークンの定義ができる機能を表しています。
...
前述の注意点から、UIライブラリを導入しないで、React Nativeのコンポーネントにテーマ機能を追加できる[@shopify/restyle](https://github.com/Shopify/restyle)[^2]の導入も検討しました。
...
上記方針でアプリの一部画面を作成した所、[UIライブラリの共通点](#UIライブラリの共通点)に記載したような場面は少なくなりました。基本的には自分たちで作成した部品であるため、各コンポーネントに設定されているスタイルを把握できています。そのため、独自にデザインされたアプリを作成する場合は、この方針との相性が良いと感じています。
