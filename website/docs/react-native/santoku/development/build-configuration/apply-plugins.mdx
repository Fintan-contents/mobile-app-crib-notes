...
| [withAndroidRemoveUsesClearTextTrafficForRelease](https://github.com/{@inject:organization}/mobile-app-crib-notes/tree/master/example-app/SantokuApp/config/plugin/src/android/withAndroidRemoveUsesClearTextTrafficForRelease.ts) | Android | stg<br/>prod | リリースビルドでは、`usesCleartextTraffic`の設定を削除します。 |
...
| [withIosDisabledATS](https://github.com/{@inject:organization}/mobile-app-crib-notes/tree/master/example-app/SantokuApp/config/plugin/src/ios/withIosDisabledATS.ts) | iOS | local | ATS（App Transport Security）を無効化します。|
