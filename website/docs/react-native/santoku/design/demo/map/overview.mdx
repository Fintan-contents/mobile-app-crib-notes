...
- [Googleマップ（iOS/Android）](https://developers.google.com/maps/documentation/ios-sdk?hl=ja)

ただし、iOSではGoogleマップの使用に制限があります。
詳細は、[iOSでGoogleマップを使用するときの注意点](#iosでgoogleマップを使用するときの注意点)を参照してください。

そのため、このアプリではiOSはMapKit、AndroidはGoogleマップを使用した地図表示のみ対応しています。
...
このアプリで作成している地図表示の共通部品（[MapView](https://github.com/{@inject:organization}/mobile-app-crib-notes/tree/master/example-app/SantokuApp/src/bases/ui/map/MapView.tsx)）ではiOSでGoogleマップを使うことはできない仕様になっています。
iOSでGoogleマップを使用する場合、共通部品は使用せず`react-native-maps`からコンポーネントをインポートしてください。
...
そのため、このアプリではiOSでGoogleマップを表示できない仕様としています。
