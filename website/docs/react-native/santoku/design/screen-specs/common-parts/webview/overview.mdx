...
※ onErrorとonHttpError時の画面を変更する場合は、個別の画面設計書に記載します。
...
| O2  | エラーメッセージ | Snackbar  | メッセージキー：`app.webview.onError`（※2） | React Native WebViewでonErrorが呼び出された、かつ親画面からプロパティ：onErrorが指定されていない場合のみ表示 |
...
|:----|:------------|:-----------|:----------|:----------------|-------------------------------------------|
| T1  | 取得したコンテンツ | WebView    | スクロール | 最底部に到達（※1）  |                                           |
...
- 最底部に到達
  - （初回のみ）プロパティ：onceScrollEndで渡された処理を実行します。
  - プロパティ：onScrollEndで渡された処理を実行します。

## プロパティ

| 名称           | 必須 | 型     | 説明                                                   |
|:--------------|:-----|:-------|:------------------------------------------------------|
| onScrollEnd   | -    | function  | イベント：最底部に到達が発生した時に実行する処理           |
| onceScrollEnd | -    | function  | イベント：最底部に到達の初回発生時のみ実行する処理           |
