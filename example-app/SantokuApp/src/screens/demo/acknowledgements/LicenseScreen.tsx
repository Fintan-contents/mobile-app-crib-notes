...
import {useLicenseScreenUseCase} from './useLicenseScreenUseCase';

const ScreenName = 'License';
const Screen: React.FC<NativeStackScreenProps<DemoStackParamList, typeof ScreenName>> = ({route}) => {
  const dependency = route.params.dependency;
  const {
    licenseContentText,
    ...
    openRepositoryLink,
  } = useLicenseScreenUseCase(dependency);
...
export const LicenseScreen: NativeStackScreenConfig<DemoStackParamList, typeof ScreenName> = {
  name: ScreenName,
  component: React.memo(Screen),
  options: {
