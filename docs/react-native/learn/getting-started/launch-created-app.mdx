...
iOSアプリはiPhoneシミュレータ、AndroidアプリはAndroidエミュレータで起動するのが基本です。また、今回作成したプロジェクトはExpoを利用しているので、Expo Goというアプリを利用して実行することもできます。
...
ブラウザがうまく起動せず開発者ツールの画面が自動的に開かれなかったとしても、[http://localhost:19002](http://localhost:19002)にアクセスすれば表示することができます。

なお、他のプロセスなどによってこれらのポートがすでに利用されている場合は、自動的に異なるポートが選択されて起動します。その場合でも、次のように表示されているログからURLを確認して開発者ツールにアクセスできます。
```
...
`npm run android` と実行することで、Androidのエミュレータが起動しアプリが実行されます。

</TabItem>
<TabItem value="iphone">

`npm run ios` と実行することで、iPhoneのシミュレータが起動しアプリが実行されます。
